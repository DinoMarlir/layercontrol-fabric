...
public class CustomRightParrotLayer<T extends Player> extends RenderLayer<T, PlayerModel<T>> {
    private final ParrotModel model;

    public CustomRightParrotLayer(RenderLayerParent<T, PlayerModel<T>> renderLayerParent, EntityModelSet entityModelSet) {
    ...
    private void render(PoseStack matrices, MultiBufferSource vertexConsumers, int light, T player, float limbAngle, float limbDistance, float headYaw, float headPitch, boolean leftShoulder) {
        float multiplier = ClothConfigManager.INSTANCE.getConfigFor(player.getUUID()).getParrotScale();
        matrices.pushPose();
        ...
        VertexConsumer vertexConsumer = vertexConsumers.getBuffer(this.model.renderType(ParrotRenderer.getVariantTexture(Parrot.Variant.values()[ClothConfigManager.INSTANCE.getConfigFor(player.getUUID()).getParrotVariantRight()])));
